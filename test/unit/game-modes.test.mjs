...
test("Game initialization: throws error if adapters are missing or incomplete", () => {
  const expectedMessage = "Game requires valid adapters: inputHandler, storageAdapter, and canvasAdapter must be provided.";

  // Scenario 1: No adapters object passed
  assert.throws(() => {
    new Game({ startLevel: 0 });
  }, {
    name: "Error",
    message: expectedMessage
  });

  // Scenario 2: Missing inputHandler
  assert.throws(() => {
    new Game({
      startLevel: 0,
      adapters: {
  ...
  // Scenario 3: Missing storageAdapter
  assert.throws(() => {
    new Game({
      startLevel: 0,
      adapters: {
  ...
  // Scenario 4: Missing canvasAdapter
  assert.throws(() => {
    new Game({
      startLevel: 0,
      adapters: {
...
test("ProgressManager: starts with session defaults", () => {
  const pg = new Game({ startLevel: 0, adapters: createFakeAdapters() });
  try {
...
test("Capture mode: sets mechanics.capture", () => {
  const cap = new Game({ startLevel: 0, mode: "infinite-capture", adapters: createFakeAdapters() });
  try {
...
test("League end: onLeagueEnd with ranking after surpassing the length", () => {
  const lg = new Game({ startLevel: 0, mode: "league-death", leagueLength: 3, adapters: createFakeAdapters() });
  try {
