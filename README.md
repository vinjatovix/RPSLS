...
- **Particle Object Pooling (Optimization):** Employs a zero-churn, class-agnostic object pooling system (`ParticlePool`) for high-volume combat particles (`BloodParticles`, `PowerUpBurst`, `FloatingText`). Reuses inactive instances from pre-allocated pools and recycles the oldest active particles when the pool limit is reached. This keeps heap allocation flat and prevents micro-stuttering from garbage collection sweeps during high-density combat.
